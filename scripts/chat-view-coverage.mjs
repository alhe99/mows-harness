...
const MUTATIONS = [
  ...
  { id: 'R2', file: 'view', desc: 'a missing message is put back but never reported, so nothing is said',
    find: '  return { turns: out, missingUsers, retired, salvagedReply };',
    repl: '  return { turns: out, missingUsers: [], retired, salvagedReply };' },
  { id: 'R3', file: 'view', desc: 'the transcript is searched by membership, not counted',
    find: "  const countOf = s => turns.reduce((n, t) => n + (t && t.role === 'user' && txt(t) === s ? 1 : 0), 0);",
    repl: "  const countOf = s => turns.some(t => t && t.role === 'user' && txt(t) === s) ? 99 : 0;" },
  ...
  // Round 2. R10 is review F1 itself: membership matching, which absorbs a repeated question into
  ...
  { id: 'D5', file: 'check', desc: 'detector blind to protocol-relative destinations again (F5)',
    find: "      if (/^[/\\\\]{2}/.test(v)) { bad.push('url:' + val); continue; }\n", repl: '' },
  { id: 'D6', file: 'check', desc: 'detector widened to flag any leading slash, so relative paths cry wolf',
    find: "      if (/^[/\\\\]{2}/.test(v)) { bad.push('url:' + val); continue; }",
    repl: "      if (/^[/\\\\]/.test(v)) { bad.push('url:' + val); continue; }" },
