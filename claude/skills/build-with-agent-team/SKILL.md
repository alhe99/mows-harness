...
**MANDATORY: You MUST use TeamCreate to create a team. Do NOT use plain Task sub-agents. This is NON-NEGOTIABLE regardless of project complexity or repo independence.**

1. Call `TeamCreate` with a descriptive `team_name` based on the plan
2. All agents MUST be spawned via the `Task` tool with `team_name` parameter set to the team name AND `name` parameter set to the agent's name — this makes them join the team as tmux-backed teammates
3. Do NOT spawn agents as plain Task sub-agents without `team_name` — this defeats the purpose of agent teams
...
**HARD RULES:**
- ALWAYS use `TeamCreate` first — no exceptions
- ALWAYS pass `team_name` and `name` when spawning teammates via Task
- NEVER use plain Task sub-agents as a shortcut — even if repos are independent
- NEVER implement code yourself — stay as the coordinator/lead
