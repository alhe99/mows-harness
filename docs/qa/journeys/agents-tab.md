...
4. Click `harness-reviewer`. Expect URL `/agents/harness-reviewer`, a "Runs" list with at least one row (if Phase 3 has run) each linking to `/agents/harness-reviewer/<run_id>`, a "Next" line, and buttons "Run now" and "Pause"/"Resume" — a "Stop" button too, but only if the state pill reads `working` (it's absent for every other state).
5. Click the newest run. Expect the assistant text of that run rendered inside a `<div class="m claude">` message bubble (a `<pre>` inside it, not an `<article>`) and a status block with `state`, `cost_usd`, `turns`.
