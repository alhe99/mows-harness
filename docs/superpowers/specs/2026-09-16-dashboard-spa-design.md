...
| 3 | **Backpressure.** *"A slow client must not stall the run. The writer drops to 'coalesce mode' for a client whose socket is not draining … The run itself never blocks on a browser."* | Not built — `coalesce`, `backpressure`, `writableLength`, `highWaterMark` and `writableNeedDrain` have zero matches across `infra/dashboard/` and `agents/bin/`; `streamWrite` is a bare `res.write` in a `try`. **This one WAS a conscious decision**, unlike everything else on this list: the plan records it as *"Gap accepted"* with the reason — a chat turn is capped at `$0.25` and six turns, so a single turn's volume is bounded, and if a slow client is ever seen stalling a run this is the first thing to add. Listed here anyway, because this ADDENDUM is introduced as the thing to read before believing a requirement shipped, and a reader who consults it and does not find backpressure concludes it did. |
| 5 | **Prompt.** *"Shown read-only, collapsed to ~6 lines with an expand."* | Not built, and until the final re-review nobody had recorded it — not the plan, not nine task reviews, not the whole-branch review. `prompt` has zero matches in either capability module; the detail endpoint returns `{name, recs, events, total, cost7d, timers, timer, capability}` and no prompt; and **nothing in `lite.mjs` ever reads an agent file's body at all** — `agentFrontmatter` shells out to `mows-agent-meta json`, which returns frontmatter only. The server-rendered twin does not show it either, so this is unbuilt rather than regressed. It is the paragraph that answers "what is this agent actually instructed to do", which is the natural companion to a panel titled *What this agent can do*. |
| 6 | Step 2 calls `/ui/agents/<name>/<run_id>` *"the run stream"* | `RunView` is a one-shot `getJSON` with no subscription. Borderline rather than a gap: §3 defines no run topic for it to subscribe to, and the server-rendered twin is equally static, so nothing was regressed and no other part of the spec asks for live run streaming. Recorded so the word "stream" in §6 does not read as a shipped feature. |
...
**One caveat on the framing above.** "Nobody decided to defer them" is true of every row except
§3's backpressure, which the plan accepted explicitly and with a reason. It is on the list anyway;
a reader checking whether a requirement shipped is not helped by leaving out the one that was
deferred on purpose.
